...
function testimonial_itemYTShortcode($atts, $content = null){
	...
	if($avatar != '') {
        ...
        $testimonial_avatar .='<img src="' . $avatar . '" alt="'.$atts['author'].'" style="border-radius:50%; width:auto; margin:0 auto; max-width:150px; max-height:150px"/> ';
    };
	...
	$testimonial_item .= '<div class="content_testimon" style="'.($avatar != '' ? "margin-top:15px;" : "").' text-align:center;'.$css.'"><div class="content">'.parse_shortcode(str_replace(array("<br/>", "<br>", "<br />"), " ", $content)).'</div>';
	$testimonial_item .= '<span class="author">'.$author.'</span> - <small>'.$position.'</small>';
