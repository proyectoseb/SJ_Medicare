...
if (!class_exists('BaseNewsHelper')){
	/**
	 * BaseHelper for com_content only.
	 */
	abstract class BaseNewsHelper{
		/**
						...
						switch ($type){
							...
							case 'image_fulltext':
								if ( isset($images_data) && isset($images_data[$type]) && !empty($images_data[$type])){
									...
									if(file_exists($image['src']) || @getimagesize($image['src']) !== false)
							...
							case 'external':
								...
								for ($i=0; $i<count($files); $i++){
									if(file_exists($files[$i]) || @getimagesize($files[$i]) !== false)
