...
if (!class_exists('ContentResponsiveListingBaseHelper')){
	/**
	 * BaseHelper for com_content only.
	 */
	abstract class ContentResponsiveListingBaseHelper{
		/**
						...
						switch ($type){
							...
							case 'image_fulltext':
								if ( isset($images_data) && isset($images_data[$type]) && !empty($images_data[$type])){
									...
									if(@GetImageSize($image['src']) || file_exists($image['src'])){
							...
							case 'inline_fulltext':
								...
								for ($i=0; $i<count($inline_images); $i++){
									if(@GetImageSize($inline_images[$i]['src']) || file_exists($inline_images[$i]['src'])) {
